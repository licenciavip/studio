...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  // ── Contadores de pendientes (consultas de un solo campo, sin índice compuesto) ──
  const upQ = useMemo(() => (firestore ? query(collection(firestore, "paymentOrders"), where("status", "==", "uploaded")) : null), [firestore]);
  const { data: uploaded } = useCollection<PaymentOrder>(upQ);
  const wQ = useMemo(() => (firestore ? query(collection(firestore, "withdrawals"), where("status", "==", "pending")) : null), [firestore]);
  const { data: wPend } = useCollection<Withdrawal>(wQ);
  const dQ = useMemo(() => (firestore ? query(collection(firestore, "disputes"), where("status", "==", "open")) : null), [firestore]);
  const { data: dOpen } = useCollection<Dispute>(dQ);
  const gQ = useMemo(() => (firestore ? query(collection(firestore, "groups"), where("approval", "==", "pending")) : null), [firestore]);
  const { data: gPend } = useCollection<GroupDoc>(gQ);

  const recargas = (uploaded ?? []).filter((p) => p.type === "wallet_recharge").length;
  const pagos = (uploaded ?? []).length;
  const retiros = (wPend ?? []).length;
  const disputas = (dOpen ?? []).length;
  const suscripciones = (gPend ?? []).length;
  const totalPend = recargas + retiros + (pagos - recargas) + disputas + suscripciones;

  const badges: Record<string, number> = {
    "/admin": totalPend,
    "/admin/recargas": recargas,
    "/admin/pagos": pagos,
  ...
  const SidebarContent = (
    <div className="flex h-full flex-col">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive(href) ? "bg-white/10 text-white" : "text-white/55 hover:bg-white/[0.06] hover:text-white/90"
              )}
            >
              <Icon className="h-4 w-4 shrink-0" strokeWidth={2} />
              <span className="flex-1">{label}</span>
              {count > 0 && (
                <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-danger px-1.5 text-[10px] font-black text-white">
                  {count > 99 ? "99+" : count}
                </span>
              )}
            </Link>
      ...
      <div className="border-t border-white/10 p-3">
        ...
        <button onClick={handleLogout} className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold text-danger transition-colors hover:bg-danger/10">
    ...
    <div className="min-h-screen bg-[#0b0e14] text-white">
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-64 border-r border-white/10 bg-[#11151f]">{SidebarContent}</aside>
        </div>
      )}
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-white/10 bg-[#0b0e14]/80 px-4 py-3 backdrop-blur sm:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setOpen(true)} className="relative rounded-lg p-1.5 text-white/70 hover:bg-white/10 lg:hidden" aria-label="Abrir menú">
              <Menu className="h-5 w-5" />
              {totalPend > 0 && <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full bg-danger" />}
            </button>
        ...
        {open && (
          <button onClick={() => setOpen(false)} className="fixed right-4 top-3 z-[60] rounded-lg p-1.5 text-white/70 hover:bg-white/10 lg:hidden" aria-label="Cerrar menú">
            <X className="h-5 w-5" />
          </button>
        )}
