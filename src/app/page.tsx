...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen pt-20">
      <main className="flex-grow w-full max-w-xl mx-auto pb-32 px-4 space-y-10">
        ...
        <section className="space-y-6">
          ...
          <Link href="/compartir" className="block group">
            <div className="relative overflow-hidden w-full h-16 bg-gradient-to-r from-[#4343d5] to-[#ff6b6b] rounded-[2rem] flex items-center justify-center gap-3 shadow-xl shadow-primary/20 transition-all duration-500 hover:scale-[1.02] active:scale-[0.98]">
        ...
        <section className="space-y-4">
          ...
          <div className="glass-card rounded-[2.5rem] p-5 flex items-center justify-between transition-all hover:bg-white/60 group">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-[1.2rem] bg-primary/10 flex items-center justify-center shadow-inner overflow-hidden border border-white/20">
        ...
        <section className="space-y-4">
          ...
          {groups.slice(0, 1).map((group) => (
            <Link href={`/mis-grupos/${group.id}`} key={group.id} className="block group">
              <div className="glass-card rounded-[2.5rem] p-5 flex items-center justify-between hover:bg-white/60 transition-all">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-[1.2rem] bg-surface-container flex items-center justify-center border border-white/20">
                ...
                {services.map((service) => {
                  ...
                  return (
                    <Link href={`/explorar/all/${service.id}`} key={service.id} className="block group">
                      <div 
                        className={cn(
                          "relative rounded-[2rem] p-3 aspect-[4/5] flex flex-col justify-between transition-all duration-500 hover:scale-[1.05] hover:shadow-xl active:scale-95 overflow-hidden",
