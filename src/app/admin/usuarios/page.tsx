  ...
  return (
    <div>
        ...
        {users?.map((u) => (
          <AdminCard key={u.id} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {u.role === "admin" && (
                <span className="rounded-full bg-primary/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-primary">Admin</span>
              )}
              {u.blocked && (
                <span className="rounded-full bg-danger/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-danger">Bloqueado</span>
              )}
            </div>
          </AdminCard>
        ))}
      ...
      <p className="mt-4 text-[11px] text-white/30">
        Las acciones (bloquear, ajustar saldo, dar permisos de admin) se conectarán con operaciones seguras de backend.
      </p>
    </div>
  );
}
